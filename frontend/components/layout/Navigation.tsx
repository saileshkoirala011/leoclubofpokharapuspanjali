...
export default function Navigation() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-[72px] flex items-center justify-between gap-6">
        ...
        <Link href="/" className="flex items-center gap-3 flex-shrink-0 group">
          ...
          <div className="hidden sm:flex flex-col leading-none gap-0.5">
            ...
            <span className={`text-[14px] font-black font-display tracking-tight transition-colors duration-300 ${solid ? 'text-[#1B3A6B]' : 'text-white'}`}>
              Puspanjali
            </span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link
            ...
            className={`btn btn-primary btn-sm ${!solid ? 'shadow-[0_4px_20px_rgba(0,0,0,0.25)]' : ''}`}
          >
            Join Now
      ...
      {/* Mobile drawer */}
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-[480px] opacity-100' : 'max-h-0 opacity-0'}`}>
